...
export interface PluginFieldMapping {
     ...
     */
    taskField: 'isDone' | 'title' | 'notes' | 'dueDay' | 'dueWithTime' | 'timeEstimate' | 'tagIds';
    issueField: string;
    ...
    toIssueValue(taskValue: unknown, ctx: {
        issueId: string;
        issueNumber?: number;
    }): unknown;
    toTaskValue(issueValue: unknown, ctx: {
        issueId: string;
        issueNumber?: number;
    }): unknown;
}
export interface PluginFormField {
    ...
    loadOptions?(config: Record<string, unknown>, http: PluginHttp): Promise<{
        label: string;
        value: string;
    }[]>;
}
...
export interface PluginHttp {
    ...
    request<T = unknown>(method: string, url: string, body?: unknown, options?: PluginHttpOptions): Promise<T>;
}
export interface IssueProviderPluginDefinition {
    configFields: PluginFormField[];
    getHeaders(config: Record<string, unknown>): Record<string, string> | Promise<Record<string, string>>;
    searchIssues(searchTerm: string, config: Record<string, unknown>, http: PluginHttp): Promise<PluginSearchResult[]>;
    ...
    getNewIssuesForBacklog?(config: Record<string, unknown>, http: PluginHttp): Promise<PluginSearchResult[]>;
    issueDisplay: PluginIssueField[];
    commentsConfig?: PluginCommentsConfig;
    fieldMappings?: PluginFieldMapping[];
    updateIssue?(id: string, changes: Record<string, unknown>, config: Record<string, unknown>, http: PluginHttp): Promise<void>;
    createIssue?(title: string, config: Record<string, unknown>, http: PluginHttp): Promise<{
        issueId: string;
        issueNumber?: number;
        issueData: PluginIssue;
    }>;
    ...
    timeBlock?: {
        ...
        upsertEvent(taskId: string, eventData: {
            title: string;
            dueWithTime: number;
            durationMs: number;
            isDone: boolean;
        }, config: Record<string, unknown>, http: PluginHttp): Promise<void>;
